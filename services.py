...
def edit_multiple_images(images_bytes: list[bytes], prompt: str = ""):
    """
    Takes multiple image bytes and a prompt, and returns the modified image bytes.
    ...
    try:
        ...
        logger.info(f"Processing multiple images edit request with prompt: {prompt[:100]}...")
        
        # Open and convert all images to PIL Image objects
        pil_images = []
        for idx, img_bytes in enumerate(images_bytes):
            try:
                image = Image.open(BytesIO(img_bytes))
                logger.info(f"Successfully opened image{idx+1}: {image.format}, size: {image.size}, mode: {image.mode}")
                
                # Convert to RGB if needed
                if image.mode != "RGB":
                    logger.info(f"Converting image{idx+1} from {image.mode} to RGB")
                    image = image.convert("RGB")
                
                pil_images.append(image)
            except Exception as img_error:
                logger.error(f"Failed to process image {idx+1}: {str(img_error)}")
                raise ValueError(f"Invalid image format for image {idx+1}: {str(img_error)}")
        
        # Use the gemini model
        model_name = "gemini-2.5-flash-image-preview"
        logger.info(f"Using model: {model_name} with {len(pil_images)} images")
        ...
        try:
            ...
            logger.info(f"Generating content with {len(pil_images)} images...")
        ...
        logger.exception("Error in edit_multiple_images:")
        return None
